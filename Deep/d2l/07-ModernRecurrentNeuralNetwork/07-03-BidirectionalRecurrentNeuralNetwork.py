...
class RNN(nn.Module):
    def __init__(self, vocab_size, num_hiddens, bidirectional=False):
        super().__init__()
...
model = RNN(vocab_size, num_hiddens, bidirectional=True)
model.rnn = nn.LSTM(num_inputs, num_hiddens, num_layers, bidirectional=True) # 多层双向 LSTM
